...
export interface AuthClassPolicy {
  ...
  // the route-policy freeze. OPERATOR_SESSION/PUBLIC freeze only the "never 403" status
  // invariant here; their wider code taxonomy belongs to the admin-auth concern.
...
// its collapse is fully frozen. OPERATOR_SESSION freezes only status=401 (never 403); its
// specific admin-auth codes are deferred. PUBLIC never authenticates.
