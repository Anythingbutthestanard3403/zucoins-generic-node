...
export const SCHEMA_PROOF_BODY_STORE_OBLIGATIONS = [
  ...
  "cross-call atomicity: the frozen ProofBodyStore port exposes no transaction boundary, so the composition root MUST run persistProofBody's find-then-insert idempotency critical section under a lock SCOPED TO THE IDEMPOTENCY TUPLE (tenant_id, operation_id, idempotency_key) -- NOT a per-(path_proof_id) advisory lock -- and MUST separately serialize the per-tenant cap check-then-increment on the tenant counter (mirroring the observation writer's (observer_id, wallet_public_key) advisory lock in SHAPE only, never in granularity); OR the composition root MUST explicitly accept and document the residuals below as an intentional relaxation.",
  "residual (NOT closed by a per-path_proof_id lock alone): the idempotency UNIQUE is tenant-scoped (tenant_id, operation_id, idempotency_key), a granularity NARROWER than path_proof_id -- two concurrent same-key requests submitted at different path_proof_id values take different locks and both reach the constraint; the per-tenant sighting cap (countSightingsByTenant / MAX_SIGHTINGS_PER_TENANT) reads a tenant-wide counter that a per-path_proof_id lock never serializes, so two concurrent same-tenant requests at different slots can both read cap-1 and both increment, overshooting the cap by up to the concurrency factor. The per-tenant cap is therefore a soft DoS bound, not a claim that the caps 'cannot be raced past' under a per-path_proof_id lock alone.",
