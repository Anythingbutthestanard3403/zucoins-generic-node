-- Reporting persistence: the database-wide conventions and scalar domains, the
-- enumerations and reporting_logical_fingerprint, and the reporting registries
-- (nodes/implementers/signing keys, reporting nonce burns, the lifecycle
...
--
-- Frozen reporting-persistence schema contract (the durable-store slice). This file is
-- CONTRACT TEXT: it is executed only by the schema-apply phase against a live database; nothing
-- in packages/node-core runs it and node-core gains no database dependency from it. It mirrors
-- custody-eligibility.sql in this same directory. The byte-identical executable copy lives at
-- apps/generic-node/drizzle/0000_reporting_persistence.sql (this is a clean-room build, so
-- the migration numbering starts at 0000).
--
...
--
-- Scope note: the durable store reads/writes the reporting nonce-burn, restore-state,
-- lifecycle-head/state, and mutation-idempotency tables. Those tables' frozen REFERENCES clauses
-- require their parent registry tables (nodes, implementers, reporting_key_bootstrap_evidence,
...
-- NodeEventVerificationStore durable adapter is deliberately out of scope here.
...
-- ===== Database-wide scalar domains the reporting tables depend on =====
...
-- ===== Enumerations the reporting tables use + the logical-fingerprint function =====
...
-- ===== Nodes, implementers, and signing keys =====
