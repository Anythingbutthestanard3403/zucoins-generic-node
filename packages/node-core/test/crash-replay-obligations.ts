...
export const CRASH_REPLAY_PERSISTENCE_OBLIGATIONS = [
  ...
  "redelivery-counter UPDATE semantics under the byte-immutability enforcement (the signed-byte columns): the harness models the column-restricted regime only — no trigger DDL is frozen; trigger installation and the live rejection of a signed-byte UPDATE are not proven here; discharged against a real database with the regime-guard installation obligation already inventoried in SCHEMA_TRANSACTION_MATERIAL_OBLIGATIONS.",
