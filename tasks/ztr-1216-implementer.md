...
**Head SHA (PR tip):** see `gh pr view 89 --json headRefOid`
**Code commit:** 591510856a0d16c7bc8e7218c051f23640e1fbd5
...
1. **Rate-limit** — packages/node-core/src/http/origin-relay-rate-limit.ts
   ...
   - Shed still answers 204 + onCandidateIntakeRefused("relay","rate_limited")
...
3. **Ops note** — docs/operations/README.md section Candidate-intake backlog
   - Restart drops in-memory inbox; reconcile re-detects

## AC
| # | Status |
| --- | --- |
| 1 relay rate-limited + test + non-oracular | satisfied |
| 2 backlog gauge scraped + test | satisfied |
| 3 ops note restart/reconcile | satisfied |
| 4 no frozen-ops change; 1188 caps | satisfied |

## Evidence at code commit 5915108
- tsc -b clean
